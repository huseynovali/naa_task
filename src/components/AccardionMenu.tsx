      ...
      {menus?.map((menuItem) => (
        <div key={menuItem.title} className="mb-2">
          <button
            ...
            className={`${
              openMenus.includes(menuItem.title)
                ? "bg-[#243C7B] text-white"
                : "bg-white text-[#787486]"
            } flex items-center justify-between w-full p-5 rounded-lg hover:bg-[#28375c] hover:text-white transition-colors`}
          >
            <div className="flex items-center justify-between w-full">
              ...
              <div
                className={
                  openMenus.includes(menuItem.title) ? "" : "rotate-180"
                }
              >
                <svg
                  width="16"
            ...
            <div className="p-5 mt-2 space-y-1 bg-white rounded-xl border border-[#F7F7F7]">
              {menuItem.links.map((link) => (
                <Link
                  key={link.path}
                  to={link.path}
                  className="block px-4 py-2 text-[#787486] hover:text-[#243C7B] rounded"
                >
                  {link.name}
                </Link>
              ))}
